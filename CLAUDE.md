...
**MVVM, offline-first, single-Activity.**

- `MainActivity` holds one `SpendViewModel` (AndroidViewModel). All navigation is state-based — there is no NavController. `ActiveView` enum (`DASHBOARD`, `HISTORY`, `LEND_BORROW`, `ADD_SPEND`) drives `AnimatedContent` in `MainContainer`.
